...
exports.revertTicket = revertTicket;
...
async function validateTicket(ticketId, stallId, operatorId, items) {
    ...
    try {
        ...
        for (const item of items) {
            ...
            const productRes = await client.query('SELECT stock FROM products WHERE product_id = $1 AND stall_id = $2 FOR UPDATE', [item.productId, stallId]);
        ...
        await client.query(`INSERT INTO tickets (ticket_id, code, total, status, operator_id, stall_id) 
       VALUES ($1, $2, $3, $4, $5, $6)`, [ticketId, code, total, 'validated', operatorId, stallId]);
        // Insere itens do ticket
        for (const item of items) {
            await client.query('INSERT INTO ticket_items (ticket_id, product_id, quantity, unit_price) VALUES ($1, $2, $3, $4)', [ticketId, item.productId, item.quantity, item.unitPrice]);
        }
...
async function revertTicket(ticketId) {
    const client = await exports.db.connect();
    try {
        await client.query('BEGIN');
        // Bloqueia o ticket contra estornos concorrentes
        const ticketRes = await client.query('SELECT status FROM tickets WHERE ticket_id = $1 FOR UPDATE', [ticketId]);
        ...
        // Puxa itens
        const itemsRes = await client.query('SELECT product_id, quantity FROM ticket_items WHERE ticket_id = $1', [ticketId]);
        ...
        // Atualiza o ticket
        await client.query('UPDATE tickets SET status = $1 WHERE ticket_id = $2', ['reverted', ticketId]);
        await client.query('COMMIT');
    }
    catch (error) {
        await client.query('ROLLBACK');
